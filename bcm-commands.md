Apos a instalaçao, verificar as informações de linceças:
```bash
[root@bcmhead ~]# cmsh -c "main licenseinfo"
[root@bcmhead ~]# request-license
```
Listando imagens:
```bash
...
```
Criando/clonando categorias:

```bash
[root@bcmhead ~]# cmsh -c "category; clone default login; commit"
```

Assinando 
```bash
[root@bcmhead ~]# cmsh -c "category use login; set softwareimage login-image; commit"
[root@bcmhead ~]# cmsh -c "category use login; list"
```
...
```
Outra forma de instalar pacotes:
```bash
[root@bcmhead ~]# dnf install -y --installroot=/cm/images/login-image htop tmux vim git pdsh rsync
[root@bcmhead ~]# rpm --root=/cm/images/login-image --install package.rpm
